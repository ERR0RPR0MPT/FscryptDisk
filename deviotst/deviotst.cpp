...
int __cdecl wmain(int argc, LPWSTR *argv)
{
  ...
  if (argc < 4)
  {
    ULONGLONG req = FSCRYPTDPROXY_REQ_INFO;
    ...
    FSCRYPTDPROXY_INFO_RESP info_resp = {0};
  ...
  {
    FSCRYPTDPROXY_READ_REQ read_req;
  ...
  {
    FSCRYPTDPROXY_WRITE_REQ write_req;
    ...
    __declspec(noreturn) void __cdecl wmainCRTStartup()
{
  ...
  if (argv == NULL)
  {
    MessageBoxA(NULL, "This program requires Windows NT/2000/XP.", "FscryptDisk",
                MB_ICONSTOP);
    ExitProcess((UINT)-1);
  }
